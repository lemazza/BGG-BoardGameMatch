...
function displayResults(collection) {
  ...
  console.log(typeof topGameCollection2);
...
function getMoreGameInfo (array) {
  ...
  let settingsObject = {
      url: `https://www.boardgamegeek.com/xmlapi/boardgame/${idArray.join(',')}`,
      type: "GET",
      dataType: "xml",
      data: {
        stats: 1
...
function getTopGames () {
  ...
  Promise.all(promiseArray).then(function() {
    getMoreGameInfo(topGameIDs.slice(topGameIDs.length*13/16, topGameIDs.length*14/16));
  });
}
